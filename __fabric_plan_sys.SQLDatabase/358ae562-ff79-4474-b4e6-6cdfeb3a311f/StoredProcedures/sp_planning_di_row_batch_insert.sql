                                  ...
                                  CREATE PROCEDURE [358ae562-ff79-4474-b4e6-6cdfeb3a311f].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [358ae562-ff79-4474-b4e6-6cdfeb3a311f].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [358ae562-ff79-4474-b4e6-6cdfeb3a311f].data_input_row (id, name, rowMeta, visualRowConfigId, dimensionId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId,temp.dimensionId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [358ae562-ff79-4474-b4e6-6cdfeb3a311f].' + @TempTableName + ' as temp;';
